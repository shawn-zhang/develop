...
WaveGenerator::WaveGenerator(string content, double freqOfTone):mSendCount(1),mLevelCount(1),mContent(content), mSampleRate(44100),mNumChannel(2),mBitsPerSample(16),mFreqOfTone(20000)
{
    if (freqOfTone > 200)
    ...
    mSendCount = strtol(content.substr(0, 2).c_str(), NULL, 16);
    mLevelCount = strtol(content.substr(2, 4).c_str(), NULL, 16);
    mLevel = new int[mLevelCount];
    int len = content.length();

    for (int i = 6, j = 0; i < len && j < mLevelCount; i += 4, j++) {
        mLevel[j] = strtol(content.substr(i, 4).c_str(), NULL, 16);
    }
...
vector<uint8_t> WaveGenerator::getWaveData()
{
    ...
    PcmVec = getPcmData();
    WaveHeader header(WaveHeader::FORMAT_PCM, mNumChannel, mSampleRate, mBitsPerSample, PcmVec.size());
